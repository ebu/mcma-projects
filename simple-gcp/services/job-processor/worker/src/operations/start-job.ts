...
export async function startJob(providers: ProviderCollection, workerRequest: WorkerRequest, context: { eventId: string, dataController: DataController }) {
...
export async function startExecution(job: Job, dataController: DataController, resourceManager: ResourceManager, authProvider: AuthProvider, logger: Logger): Promise<Job> {
    ...
    try {
        ...
        for (const service of services) {
            ...
            if (service.jobType === job["@type"]) {
                ...
                if (service.jobProfileIds) {
                    for (let serviceJobProfile of service.jobProfileIds) {
        ...
        await schedulerClient.resumeJob({ name: process.env.CloudSchedulerJobName })
    } catch (error) {
