...
export class DataController {
    ...
    async queryExecutions(jobId: string, queryParameters: JobResourceQueryParameters, pageStartToken?: string): Promise<QueryResults<JobExecution>> {
        ...
        const jobPath = extractPath(jobId);
        queryParameters.path = `${jobPath}/executions`;
    ...
    async getExecutions(jobId: string): Promise<QueryResults<JobExecution>> {
        ...
        return await this.dbTable.query({ path: jobPath + "/executions" });
    }
    ...
    async createMutex(resourceId: string, mutexHolder: string): Promise<DocumentDatabaseMutex> {
        await this.init();
        const resourcePath = extractPath(resourceId);
        return this.dbTable.createMutex(resourcePath, mutexHolder);
    }
}
