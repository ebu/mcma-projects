#################################
#  aws_lambda_function : ffmpeg-service-api-handler
#################################

resource "aws_lambda_function" "ffmpeg_service_api_handler" {
  filename         = "../services/ffmpeg-service/api-handler/build/dist/lambda.zip"
  function_name    = format("%.64s", "${var.global_prefix}-ffmpeg-service-api-handler")
  role             = aws_iam_role.iam_for_exec_lambda.arn
  handler          = "index.handler"
  source_code_hash = filebase64sha256("../services/ffmpeg-service/api-handler/build/dist/lambda.zip")
  runtime          = "nodejs10.x"
  timeout          = "30"
  memory_size      = "3008"
...
resource "aws_lambda_layer_version" "ffmpeg" {
  filename         = "../services/ffmpeg-service/ffmpeg/ffmpeg.zip"
  layer_name       = "${var.global_prefix}-ffmpeg-service-ffmpeg"
  source_code_hash = filebase64sha256("../services/ffmpeg-service/ffmpeg/ffmpeg.zip")
}

resource "aws_lambda_function" "ffmpeg_service_worker" {
  filename         = "../services/ffmpeg-service/worker/build/dist/lambda.zip"
  function_name    = format("%.64s", "${var.global_prefix}-ffmpeg-service-worker")
  role             = aws_iam_role.iam_for_exec_lambda.arn
  handler          = "index.handler"
  source_code_hash = filebase64sha256("../services/ffmpeg-service/worker/build/dist/lambda.zip")
  runtime          = "nodejs10.x"
  timeout          = "900"
  memory_size      = "3008"
...
##################################
# aws_dynamodb_table : ffmpeg_service_table
##################################
...
resource "aws_api_gateway_method" "ffmpeg_service_options_method" {
  rest_api_id   = aws_api_gateway_rest_api.ffmpeg_service_api.id
...
resource "aws_api_gateway_method_response" "ffmpeg_service_options_200" {
  rest_api_id = aws_api_gateway_rest_api.ffmpeg_service_api.id
  resource_id = aws_api_gateway_resource.ffmpeg_service_api_resource.id
  http_method = aws_api_gateway_method.ffmpeg_service_options_method.http_method
  status_code = "200"
...
resource "aws_api_gateway_integration" "ffmpeg_service_options_integration" {
  rest_api_id = aws_api_gateway_rest_api.ffmpeg_service_api.id
  resource_id = aws_api_gateway_resource.ffmpeg_service_api_resource.id
  http_method = aws_api_gateway_method.ffmpeg_service_options_method.http_method
  type        = "MOCK"
...
resource "aws_api_gateway_integration_response" "ffmpeg_service_options_integration_response" {
  rest_api_id = aws_api_gateway_rest_api.ffmpeg_service_api.id
  resource_id = aws_api_gateway_resource.ffmpeg_service_api_resource.id
  http_method = aws_api_gateway_method.ffmpeg_service_options_method.http_method
  status_code = aws_api_gateway_method_response.ffmpeg_service_options_200.status_code
...
resource "aws_api_gateway_deployment" "ffmpeg_service_deployment" {
  depends_on = [
    ...
    aws_api_gateway_integration.ffmpeg_service_options_integration,
    aws_api_gateway_integration_response.ffmpeg_service_options_integration_response,
  ]
...
locals {
  ffmpeg_service_url = "https://${aws_api_gateway_rest_api.ffmpeg_service_api.id}.execute-api.${var.aws_region}.amazonaws.com/${var.environment_type}"
}
