...
resource "aws_lambda_function" "mediainfo_service_api_handler" {
  filename         = "../services/mediainfo-service/api-handler/build/dist/lambda.zip"
  function_name    = format("%.64s", "${var.global_prefix}-mediainfo-service-api-handler")
  role             = aws_iam_role.iam_for_exec_lambda.arn
  handler          = "index.handler"
  source_code_hash = filebase64sha256("../services/mediainfo-service/api-handler/build/dist/lambda.zip")
  runtime          = "nodejs10.x"
  timeout          = "30"
  memory_size      = "3008"
...
resource "aws_lambda_layer_version" "mediainfo" {
  filename         = "../services/mediainfo-service/mediainfo/MediaInfo_CLI_19.09_Lambda_AL2.zip"
  layer_name       = "${var.global_prefix}-mediainfo-service-mediainfo"
  source_code_hash = filebase64sha256("../services/mediainfo-service/mediainfo/MediaInfo_CLI_19.09_Lambda_AL2.zip")
}

resource "aws_lambda_function" "mediainfo_service_worker" {
  filename         = "../services/mediainfo-service/worker/build/dist/lambda.zip"
  function_name    = format("%.64s", "${var.global_prefix}-mediainfo-service-worker")
  role             = aws_iam_role.iam_for_exec_lambda.arn
  handler          = "index.handler"
  source_code_hash = filebase64sha256("../services/mediainfo-service/worker/build/dist/lambda.zip")
  runtime          = "nodejs10.x"
  timeout          = "900"
  memory_size      = "3008"
