...
const TEST_FILE = "../test-file.mp4";

async function sleep(timeout) {
    return new Promise((resolve) => setTimeout(() => resolve(), timeout));
}

async function uploadFileToBucket(bucket, filename) {
    ...
    const uploadParams = { Bucket: bucket, Key: "transform-input/" + path.basename(filename), Body: fileStream };
...
async function main() {
    ...
    const transformInputFile = await uploadFileToBucket(tempBucket, TEST_FILE);

    const transformOutputLocation = new AwsS3FolderLocator({
        awsS3Bucket: tempBucket,
        awsS3KeyPrefix: "transform-output/",
    });
