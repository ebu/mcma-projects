...
const loggerProvider = new AwsCloudWatchLoggerProvider("ai-workflow-311-detect-emotions-aws", process.env.LogGroupName);
...
const JOB_PROFILE_NAME = "AWSDetectEmotions";
...
export async function handler(event: InputEvent, context: Context) {
    ...
    try {
        ...
        try {
            event.parallelProgress = { "detect-emotions-aws": 20 };
        ...
        logger.error("Failed to detect emotions");
        logger.error(error.toString());
        throw new McmaException("Failed to detect emotions", error);
    } finally {
