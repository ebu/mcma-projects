...
resource "aws_lambda_function" "ame-service-api-handler" {
  ...
  source_code_hash = filebase64sha256("./../services/ame-service/api-handler/dist/lambda.zip")
  runtime          = "nodejs10.x"
  timeout          = "30"
  memory_size      = "256"
}
...
resource "aws_lambda_function" "ame-service-worker" {
  ...
  role             = aws_iam_role.iam_for_exec_lambda.arn
  handler          = "index.handler"
  source_code_hash = filebase64sha256("./../services/ame-service/worker/dist/lambda.zip")
  runtime          = "nodejs10.x"
  timeout          = "300"
  memory_size      = "3008"
}
...
resource "aws_api_gateway_deployment" "ame_service_deployment" {
  depends_on = [aws_api_gateway_integration.ame_service_api_method_integration]
