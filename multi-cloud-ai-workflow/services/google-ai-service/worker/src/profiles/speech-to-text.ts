...
import { AIJob, ConfigVariables, McmaException } from "@mcma/core";
...
async function getGoogleServiceCredentials(): Promise<any> {
    try {
        const googleServiceCredentialsS3Bucket = configVariables.get("GoogleServiceCredentialsS3Bucket");
        const googleServiceCredentialsS3Key = configVariables.get("GoogleServiceCredentialsS3Key");
